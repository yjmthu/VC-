...
#define BufSize 30				//设置存放字符的缓冲区大小
	...
	static bool nCtrlDown = false, nAltDown = false, nShiftDown = false, outRight = false, isNotAscii = false;
	...
	case WM_CHAR:				//遇到非系统字符所作的处理
	{
		...
		if (!isascii(wParam))
		{
			...
			if (gbkCode)
			{
				...
				else if ((nNumChar + 3) * nCharWidth > star[10].x)
				{
					gbkCode = 0, outRight = true, MessageBox(hWnd, "快要超出边界了，不能再输入字符了\n若需要删除字符，请用BackSpace键", NULL, MB_OK);
					break;
				}
				else
				{
		...
		else if ((nNumChar + 2) * nCharWidth > star[10].x)
		{
			if (!outRight) outRight = true, MessageBox(hWnd, "快要超出边界了，不能再输入字符了\n若需要删除字符，请用BackSpace键", NULL, MB_OK);
			break;
		}
		outRight = false;
...
#undef min
	...
	case WM_KEYDOWN:     //处理按下键消息
	{
		...
		case '1':
			...
			if (nLnHeight * 4 > star[10].y || nCharWidth * 2 * nNumChar > star[10].x)
			{
				MessageBox(hWnd, "不能再放大了!", "提示", 0);
				return TRUE;
			}
			nLnHeight *= 2;
			nCharWidth *= 2;
